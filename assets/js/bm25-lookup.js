...
(function (root, factory) {
    ...
    function classifyQuery(query) {
        ...
        const flags = {
            ...
            hasTrouble: /报错|异常|崩溃|闪退|卡死|不生效|没效果|突然|消失|解决|怎么修|怎么排查|怎么办/.test(lower)
        ...
        // Intent priority: troubleshooting > intro > howto > unknown
        let intent = 'unknown';
    ...
    Bm25LookupEngine.prototype.search = function (query, options) {
        ...
        const requireKeyTerm = expandedKeyTerms.length > 0;
        ...
        for (const item of pre) {
            ...
            let keyHit = 0;
            if (requireKeyTerm) {
                for (const kt of expandedKeyTerms) {
            ...
            // 规则：
            // - 如果识别到 keyTerm（通常是“物品/入门/弹幕”），必须至少命中一个 keyTerm，否则大幅降权
            // - 必须命中一定数量的 query token，否则降权（抵御 bucket 碰撞）
            let multiplier = 1;
            if (hit < minTokenHit) multiplier *= 0.35;
            if (requireKeyTerm && keyHit === 0) multiplier *= 0.15;

            const boosted = item.score * multiplier * (1 + Math.min(0.25, hit * 0.04));
            rescored.push({ item, score: boosted, hit, keyHit });
        }
        ...
        for (const x of rescored) {
            ...
            out.push({ ...x.item, score: x.score, __hit: x.hit, __keyHit: x.keyHit });
