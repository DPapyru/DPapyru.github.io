...
function lerpVec(a, b, t) { return vec(lerp(a.x, b.x, t), lerp(a.y, b.y, t)); }
...
function initState() {
    return {
        ...
        // 各个模式的独立状态
        dashState: { phase: 'windup', timer: 0 },
        seekState: { targetFound: false, targetId: -1 },
        boomerangState: { phase: 'out', timer: 0, startPos: vec(0, 0) },
        curveState: { angle: 0 },
        autoDemo: true,
        autoDemoTimer: 0
    };
...
export function updateAI(state, input, dt) {
    ...
    if (proj.trail.length > 40)
        proj.trail.shift();
    // UI 状态
    state.uiStates.forEach((s) => {
...
// ========== AI 更新函数 ==========
function updateBasicAI(proj, state, dt) {
    proj.timer += dt;
    // 旋转跟随速度方向
    if (len(proj.vel) > 1) {
...
function updateDashAI(proj, state, dt) {
...
function updateSeekAI(proj, state, dt) {
    // 模拟 NPC 目标
    const targetPos = vec(Math.sin(state.time * 0.5) * 150, -80 + Math.cos(state.time * 0.3) * 60);
...
function updateBoomerangAI(proj, state, dt, player) {
...
function updateCurveAI(proj, state, dt) {
...
export function render(state, input, gfx) {
    ...
    if (state.selectedMode === 'Seek') {
        ...
        g.beginPath();
        g.arc(npcScreen.x, npcScreen.y, 12, 0, Math.PI * 2);
        g.fill();
        g.fillStyle = 'rgba(255,255,255,0.7)';
        g.font = '11px system-ui, sans-serif';
    ...
    // HUD
    if (input.ui) {
        input.ui.badges.set(`Mode: ${state.selectedMode}`, state.autoDemo ? 'Auto Demo' : 'Manual', `t=${state.time.toFixed(1)}s`);
...
//# sourceMappingURL=demo-projectile-ai.js.map