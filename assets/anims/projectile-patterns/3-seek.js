...
function render(state, input, gfx, modeName) {
    ...
    const proj = state.projectiles[0];
    if (proj) {
        const projScreen = toScreen(proj.pos);
        g.strokeStyle = 'rgba(255, 80, 80, 0.3)';
        g.lineWidth = 1;
        g.setLineDash([5, 5]);
        g.beginPath();
        g.moveTo(projScreen.x, projScreen.y);
        g.lineTo(npcScreen.x, npcScreen.y);
        g.stroke();
        g.setLineDash([]);
    }
    // 拖尾
...
export function updateAI(state, input, dt) {
    if (!state.__init) {
        Object.assign(state, createState('Seek'));
        state.__init = true;
...
export function renderFn(state, input, gfx) {
    render(state, input, gfx, 'Seek: 追踪 + 平滑转向');
}
//# sourceMappingURL=3-seek.js.map