...
export function createCartesian2D(options) {
...
export function drawAxes2D(g, cs, options) {
...
export function plotFunction2D(g, cs, f, options) {
    const opts = options && typeof options === 'object' ? options : {};
    const xMin = Number.isFinite(Number(opts.xMin)) ? Number(opts.xMin) : -6;
    const xMax = Number.isFinite(Number(opts.xMax)) ? Number(opts.xMax) : 6;
...
export function fillIntegralArea2D(g, cs, f, a, b, options) {
    const opts = options && typeof options === 'object' ? options : {};
...
export function drawArrow2D(g, from, to, options) {
    const opts = options && typeof options === 'object' ? options : {};
