...
export default async function run(ctx) {
    ...
    function updateHud() {
        ...
        scene.badges.set(`Phase: ${eye.phase}`, lock ? `Mode: ${eye.mode} (LOCK)` : `Mode: ${eye.mode}`, `HP: ${(eye.hp * 100).toFixed(0)}%`);
    ...
    function updateAI(dt) {
        eye.timer += dt;
        SIM.time += dt;
        const forcedMode = scene.states.lock.get();
        if (forcedMode && eye.mode !== forcedMode)
            setMode(forcedMode);
        ...
        if (eye.mode === 'Approach') {
            ...
            if (len(toTarget) < 16 && transitionsAllowed)
                setMode('Hover');
        }
            ...
            if (eye.timer >= hoverTime && transitionsAllowed)
                setMode('ChargePrep');
        }
            ...
            eye.pos = add(eye.pos, mul(eye.vel, dt));
            if (eye.timer >= 0.55 && transitionsAllowed)
                setMode('ChargePrep');
        }
            ...
            eye.vel = mul(eye.vel, 0.84);
            eye.pos = add(eye.pos, mul(eye.vel, dt));
            if (eye.timer >= 1.2 && transitionsAllowed)
                setMode('ChargePrep');
        }
        // Update minions
    ...
    function drawOverlayText(paused) {
        if (!paused)
            return;
        g.save();
        const width = scene.canvas.width;
...
//# sourceMappingURL=demo-eoc-ai.js.map