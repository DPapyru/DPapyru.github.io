...
async function runDiagnostics() {
    ...
    try {
        const rulePayload = await languageRpc.call(MESSAGE_TYPES.DIAGNOSTICS_RULE_REQUEST, source);
        let allDiags = Array.isArray(rulePayload.diagnostics) ? rulePayload.diagnostics : [];
...
function installEditorProviders() {
    monaco.languages.registerCompletionItemProvider('csharp', {
        ...
        async provideCompletionItems(model, position) {
            ...
            const payload = await languageRpc.call(MESSAGE_TYPES.COMPLETION_REQUEST, {
                text: model.getValue(),
                offset,
                maxItems: 120
            });
    ...
    monaco.languages.registerHoverProvider('csharp', {
        async provideHover(model, position) {
            const offset = model.getOffsetAt(position);
            const payload = await languageRpc.call(MESSAGE_TYPES.HOVER_REQUEST, {
                text: model.getValue(),
                offset
            });
...
function bindUiEvents() {
    ...
    dom.btnSaveWorkspace.addEventListener('click', async function () {
        try {
            await saveWorkspace(workspaceSnapshotForSave());
...
async function bootstrap() {
    ...
    globalThis.__tmlIdeDebug = {
        ...
        setCursorAfterText(needle) {
            if (!state.editor || !state.editor.getModel()) return false;
        ...
        async requestHoverAtCursor() {
            ...
            return payload && payload.hover ? payload.hover : null;
        }
    };
