    ...
    try {
        ...
        if (config.all_files && Array.isArray(config.all_files)) {
            config.all_files.forEach(file => {
                // file.path 已经包含完整路径，不需要再添加 "site/content/" 前缀
...
function fetchDocumentInfo(url) {
    ...
    return (window.SiteConfig ? window.SiteConfig.load() : fetch('/site/content/config.json').then(response => {
        if (!response.ok) {
...
function generateUpdateCard(doc) {
    ...
    if (viewUrl.endsWith('.md') && !viewUrl.includes('viewer.html')) {
        const relativePath = viewUrl.startsWith('site/content/')
            ? viewUrl.substring('site/content/'.length)
            : viewUrl;
        viewUrl = `/site/pages/viewer.html?file=${encodeURIComponent(relativePath)}`;
    }
