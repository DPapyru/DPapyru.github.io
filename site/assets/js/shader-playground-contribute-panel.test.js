...
test('shader playground page includes embedded contribute panel assets', () => {
    const file = path.resolve('tml-ide-app/public/subapps/shader/index.html');
    const text = fs.readFileSync(file, 'utf8');

    assert.match(text, /\/tml-ide\/subapps\/assets\/css\/shader-contribute\.css/);
    ...
    assert.match(text, /\/tml-ide\/subapps\/assets\/js\/shader-contribute\.js/);
});

test('shader playground contribute button no longer navigates away', () => {
    const file = path.resolve('tml-ide-app/public/subapps/assets/js/shader-playground.js');
    const text = fs.readFileSync(file, 'utf8');
