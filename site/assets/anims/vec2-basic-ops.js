...
export function create(runtime) {
    ...
    class Vec2BasicOps {
        ...
        OnInit(ctx) {
            this._ctx = ctx;
            this._cursor = new Vector2(0.45, 0.2);
            this._mode = 0;
        }
        OnUpdate(dt) {
            if (this._ctx == null)
            ...
            if (this._ctx.Input.IsInside)
            {
                ...
                this._cursor = new Vector2((this._ctx.Input.X - centerX) / (this._ctx.Width * 0.25), (centerY - this._ctx.Input.Y) / (this._ctx.Height * 0.25));
            }
        ...
        OnRender(g) {
            ...
            let a = new Vector2(0.9, 0.35);
            let b = this._cursor;
            let result = a;
            let label = "a \u002B b";
            switch (this._mode) {
                case 1:
                    result = Vector2.Sub(a, b);
                    label = "a - b";
                    break;
                case 2:
                    result = Vector2.MulScalar(a, 1.45);
                    label = "a * 1.45";
                    break;
                default:
            ...
            g.Text("Vector2 Basic Ops", new Vector2(12, 12), new Color(225, 235, 245, 240), 13);
            g.Text("\u6A21\u5F0F: " + label, new Vector2(12, 30), new Color(160, 198, 230, 220), 12);
            g.Text("\u84DD: a  \u6A59: b  \u7EFF: result", new Vector2(12, 48), new Color(130, 150, 175, 220), 12);
        }
        OnDispose() {
        }
    }
