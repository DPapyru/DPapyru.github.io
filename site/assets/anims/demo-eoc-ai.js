...
export function create(runtime) {
    ...
    class DemoEocAi {
        constructor() {
            ...
            this._state = null;
            this._phase = null;
            this._chargeCount = null;
            this._overrideActive = null;
        ...
        OnInit(ctx) {
            this._ctx = ctx;
            this._position = new Vector2(ctx.Width * 0.5, ctx.Height * 0.4);
            this._velocity = new Vector2(0, 0);
            this._stateTimer = 0;
            this._maxLife = 2800;
            ...
            this._minionPos = [new Vector2(0, 0), new Vector2(0, 0), new Vector2(0, 0), new Vector2(0, 0), new Vector2(0, 0), new Vector2(0, 0)];
            this._minionVel = [new Vector2(0, 0), new Vector2(0, 0), new Vector2(0, 0), new Vector2(0, 0), new Vector2(0, 0), new Vector2(0, 0)];
        ...
        OnUpdate(dt) {
            ...
            {
                target = new Vector2(this._ctx.Input.X, this._ctx.Input.Y);
            }
        ...
        UpdateHover(target, dt, offset, hoverTime, speed) {
            let desired = new Vector2(target.X + offset.X, target.Y + offset.Y);
            let desiredVelocity = DemoEocAi.Scale(DemoEocAi.Normalize(new Vector2(desired.X - this._position.X, desired.Y - this._position.Y)), speed);
            this._velocity = DemoEocAi.Lerp(this._velocity, desiredVelocity, 0.08);
            this._position = new Vector2(this._position.X + this._velocity.X * dt, this._position.Y + this._velocity.Y * dt);
        ...
        UpdateCharge(target, dt, speed, chargeTime) {
            ...
            {
                this._velocity = DemoEocAi.Scale(DemoEocAi.Normalize(new Vector2(target.X - this._position.X, target.Y - this._position.Y)), speed);
            }
            this._position = new Vector2(this._position.X + this._velocity.X * dt, this._position.Y + this._velocity.Y * dt);
            this._position = DemoEocAi.ClampToArena(this._position, this._ctx.Width, this._ctx.Height, 40);
        }
        UpdateTransform(target, dt) {
            let desiredVelocity = DemoEocAi.Scale(DemoEocAi.Normalize(new Vector2(target.X - this._position.X, target.Y - this._position.Y)), 80);
        ...
        TrySpawnMinions(target, dt) {
            ...
            for (let i = 0; i < this._minionActive.length; i++)
            {
                if (!this._minionActive[i])
                {
                    ...
                    this._minionPos[i] = new Vector2(this._position.X, this._position.Y);
                    this._minionVel[i] = new Vector2(0, 0);
                    break;
                }
            }
        }
        UpdateMinions(target, dt) {
            for (let i = 0; i < this._minionActive.length; i++)
            {
                ...
                let desired = new Vector2(target.X + MathF.Cos(orbitAngle) * orbitRadius, target.Y + MathF.Sin(orbitAngle) * orbitRadius);
                let desiredVelocity = DemoEocAi.Scale(DemoEocAi.Normalize(new Vector2(desired.X - this._minionPos[i].X, desired.Y - this._minionPos[i].Y)), 140);
                this._minionVel[i] = DemoEocAi.Lerp(this._minionVel[i], desiredVelocity, 0.2);
                this._minionPos[i] = new Vector2(this._minionPos[i].X + this._minionVel[i].X * dt, this._minionPos[i].Y + this._minionVel[i].Y * dt);
            }
        }
        ResetMinions() {
            for (let i = 0; i < this._minionActive.length; i++)
            {
                ...
                this._minionPos[i] = new Vector2(0, 0);
                this._minionVel[i] = new Vector2(0, 0);
            }
        }
        DrawHealth(g) {
            let barStart = new Vector2(18, 18);
            let barWidth = 220;
            let barEnd = new Vector2(barStart.X + barWidth, barStart.Y);
            let ratio = this._maxLife <= 0 ? 0 : this._life / this._maxLife;
            let filled = new Vector2(barStart.X + barWidth * ratio, barStart.Y);
        ...
        DrawMinions(g) {
            for (let i = 0; i < this._minionActive.length; i++)
            {
                ...
                let pupil = new Vector2(pos.X + 2, pos.Y + 2);
        ...
        GetHoverOffset() {
            ...
            return new Vector2(MathF.Cos(this._ctx.Time * 0.9) * 80, -120 + bob);
        }
        GetPhase1HoverTime() {
            return 1.05;
        }
        ...
        static DrawArena(g, width, height) {
            ...
            let border = 20;
            g.Line(new Vector2(border, border), new Vector2(width - border, border), frameColor, 1);
            g.Line(new Vector2(width - border, border), new Vector2(width - border, height - border), frameColor, 1);
            g.Line(new Vector2(width - border, height - border), new Vector2(border, height - border), frameColor, 1);
            g.Line(new Vector2(border, height - border), new Vector2(border, border), frameColor, 1);
        }
        ...
        static DrawEye(g, center, target, isPhase2, time) {
            ...
            let gaze = DemoEocAi.Normalize(new Vector2(target.X - center.X, target.Y - center.Y));
            let irisCenter = new Vector2(center.X + gaze.X * bodyRadius * 0.35, center.Y + gaze.Y * bodyRadius * 0.35);
            let pupilCenter = new Vector2(irisCenter.X + gaze.X * 5, irisCenter.Y + gaze.Y * 5);
            for (let i = 0; i < 8; i++)
            {
                let angle = time * 0.9 + i * 0.78;
                let dir = new Vector2(MathF.Cos(angle), MathF.Sin(angle));
                let start = new Vector2(center.X + dir.X * bodyRadius * 0.9, center.Y + dir.Y * bodyRadius * 0.9);
                let end = new Vector2(center.X + dir.X * (bodyRadius + 10 + 6 * MathF.Sin(time + i)), center.Y + dir.Y * (bodyRadius + 10 + 6 * MathF.Sin(time + i)));
