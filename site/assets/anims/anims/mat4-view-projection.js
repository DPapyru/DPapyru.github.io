...
export function create(runtime) {
    ...
    class Mat4ViewProjection {
        ...
        OnRender(g) {
            ...
            let center = new Vector2(this._ctx.Width * 0.5, this._ctx.Height * 0.58);
            let scale = MathF.Min(this._ctx.Width, this._ctx.Height) * 0.34;
            let aspect = this._ctx.Width / MathF.Max(1, this._ctx.Height);
            let view = Matrix.Multiply(Matrix.Multiply(Matrix.CreateRotationY(this._yaw), Matrix.CreateRotationX(this._pitch)), Matrix.CreateTranslation(0, 0, 2.8));
            let projection = Matrix.CreatePerspectiveFieldOfView(1.05, aspect, 0.1, 10);
            let vp = Matrix.Multiply(projection, view);
            ...
            g.Text("Matrix View \u002B Projection", new Vector2(12, 12), new Color(225, 235, 245, 220), 13);
            g.Text(useProjection ? "\u6A21\u5F0F: \u900F\u89C6\u002B\u89C6\u56FE" : "\u6A21\u5F0F: \u4EC5\u89C6\u56FE", new Vector2(12, 30), new Color(150, 188, 224, 220), 12);
            g.Text("\u62D6\u62FD\u65CB\u8F6C\u89C6\u89D2\uFF0Cmode-select \u53EF\u5207\u6362", new Vector2(12, 48), new Color(130, 150, 175, 220), 12);
        }
        OnDispose() {
        }
        static DrawGrid(g, m, center, scale) {
            ...
            while (i <= 4) {
                let t = i * 0.25;
                Mat4ViewProjection.DrawSegment(g, Matrix.TransformVector3(m, new Vector3(t, 0, -1)), Matrix.TransformVector3(m, new Vector3(t, 0, 1)), center, scale, new Color(52, 70, 95, 120), 1);
                Mat4ViewProjection.DrawSegment(g, Matrix.TransformVector3(m, new Vector3(-1, 0, t)), Matrix.TransformVector3(m, new Vector3(1, 0, t)), center, scale, new Color(52, 70, 95, 120), 1);
                i += 1;
            }
        }
        static DrawAxis(g, m, center, scale) {
            Mat4ViewProjection.DrawArrow(g, Matrix.TransformVector3(m, new Vector3(0, 0, 0)), Matrix.TransformVector3(m, new Vector3(1.1, 0, 0)), center, scale, new Color(255, 112, 112, 230), 2);
            Mat4ViewProjection.DrawArrow(g, Matrix.TransformVector3(m, new Vector3(0, 0, 0)), Matrix.TransformVector3(m, new Vector3(0, 1.1, 0)), center, scale, new Color(125, 255, 145, 230), 2);
            Mat4ViewProjection.DrawArrow(g, Matrix.TransformVector3(m, new Vector3(0, 0, 0)), Matrix.TransformVector3(m, new Vector3(0, 0, 1.1)), center, scale, new Color(130, 176, 255, 230), 2);
        }
