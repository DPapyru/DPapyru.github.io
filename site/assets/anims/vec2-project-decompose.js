// Generated from anims/vec2-project-decompose.anim.ts
export function create(runtime) {
    ...
    class Vec2ProjectDecompose {
        ...
        OnInit(ctx) {
            this._ctx = ctx;
            this._vector = new Vector2(0.95, 0.6);
            this._mode = 0;
        }
        OnUpdate(dt) {
            if (this._ctx == null)
            ...
            if (this._ctx.Input.IsInside)
            {
                ...
                this._vector = new Vector2((this._ctx.Input.X - centerX) / (this._ctx.Width * 0.25), (centerY - this._ctx.Input.Y) / (this._ctx.Height * 0.25));
            }
        ...
        OnRender(g) {
            ...
            let axis = new Vector2(MathF.Cos(this._ctx.Time * 0.55), MathF.Sin(this._ctx.Time * 0.55));
            let axisLen = MathF.Sqrt(axis.X * axis.X + axis.Y * axis.Y);
            if (axisLen > 0.0001)
            {
                axis = Vector2.DivScalar(axis, axisLen);
            }
            let dot = this._vector.X * axis.X + this._vector.Y * axis.Y;
            let proj = Vector2.MulScalar(axis, dot);
            let rej = Vector2.Sub(this._vector, proj);
            AnimGeom.DrawAxes(g, center, scale, new Color(88, 104, 130, 220), new Color(45, 58, 82, 130));
            let axisTipA = AnimGeom.ToScreen(Vector2.MulScalar(axis, 1.4), center, scale);
            let axisTipB = AnimGeom.ToScreen(Vector2.MulScalar(axis, -1.4), center, scale);
            ...
            g.Text("Vector2 Projection \u0026 Decompose", new Vector2(12, 12), new Color(225, 235, 245, 240), 13);
            g.Text(this._mode == 1 ? "\u5206\u89E3: v = proj \u002B rej" : "\u6295\u5F71: proj(v, axis)", new Vector2(12, 30), new Color(155, 190, 225, 220), 12);
            g.Text("\u767D\u7EBF: axis  \u84DD: v  \u9EC4: proj" + (this._mode == 1 ? "  \u7EA2: rej" : ""), new Vector2(12, 48), new Color(130, 150, 175, 220), 12);
        }
        OnDispose() {
        }
    }
