...
namespace ModDocProject.Modder入门.详细文档.CSharp知识 {
    #region 元数据
    /// <summary>掌握 if/else、循环与 return，让代码“按条件运行”</summary>
    [Title("C#控制流")]
    [Tooltip("掌握 if/else、循环与 return，让代码“按条件运行”")]
    ...
    [Difficulty("beginner")]
    [Time("15分钟")]
    [PrevChapter("CSharp变量表达式")]
    [NextChapter("CSharp数组与集合")]
    #endregion
    public class CSharp控制流 {
#if DOCS
        #region 控制流是什么
...
#if DOCS
        #region break/continue：循环里的两种跳转
...
#if DOCS
        #region while：当你不知道循环次数时
        public const string DocMarkdown_1_3 = """
        `for` 适合“循环次数明确”的场景。
