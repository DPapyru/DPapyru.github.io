...
public sealed class vector_add_resolution : IAnimScript
{
    ...
    public void OnUpdate(float dt)
    {
        ...
        if (_mousePos.X != 0 || _mousePos.Y != 0)
            _center = new Vector2((_mousePos.X * 0.2f + _center.X * 1.8f) * 0.5f,(_mousePos.Y * 0.2f + _center.Y * 1.8f) * 0.5f);
    }
    ...
    public void OnRender(ICanvas2D g)
    {
        ...
        var vec2 = new Vector2(0.3f, 0.5f);

        if (_center.X != 0 || _center.Y != 0)
            vec2 = new Vector2((_center.X - center.X) / width * 6.5f, -(_center.Y - center.Y) / height * 4);
        
        // 绘制坐标系
        DrawAxes(g, center, scale * 2.5f);

        var center_x = new Vector2(center.X, center.Y - vec2.Y * height / 4);
        var center_y = new Vector2(center.X + vec2.X * width / 6.5f, center.Y);
        
        // 绘制X轴
        DrawArrow(g,center,ToScreen(new Vector2(vec2.X,0),center, scale),new Color(231,234,20,255),1.5f,8f);
        DrawArrow(g,center_x,ToScreen(new Vector2(vec2.X,0),center_x, scale),new Color(231,234,20,150),1.5f,8f);
        // 绘制Y轴
        DrawArrow(g,center,ToScreen(new Vector2(0,vec2.Y),center, scale),new Color(231,24,230,255),1.5f,8f);
        DrawArrow(g,center_y,ToScreen(new Vector2(0,vec2.Y),center_y, scale),new Color(231,24,230,150),1.5f,8f);
        // 绘制原向量
    ...
    private static void DrawAxes(ICanvas2D g, Vector2 center, float scale)
    {
        ...
        for (int i = 1; i <= 4; i++)
        {
            ...
            g.Line(new Vector2(center.X - axisLength, center.Y - offset),
                new Vector2(center.X + axisLength, center.Y - offset), gridColor, 1f
            );
            g.Line(new Vector2(center.X - axisLength, center.Y + offset),
                new Vector2(center.X + axisLength, center.Y + offset), gridColor, 1f
            );
            g.Line(new Vector2(center.X - offset, center.Y - axisLength),
                new Vector2(center.X - offset, center.Y + axisLength), gridColor, 1f
            );
            g.Line(new Vector2(center.X + offset, center.Y - axisLength),
                new Vector2(center.X + offset, center.Y + axisLength), gridColor, 1f
            );
