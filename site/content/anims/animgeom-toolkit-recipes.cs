...
public sealed class AnimgeomToolkitRecipes : IAnimScript
{
    ...
    public void OnRender(ICanvas2D g)
    {
        ...
        g.Text("AnimGeom Toolkit Recipes", new Vector2(12f, 12f), new Color(225, 235, 245, 235), 13f);
        g.Text(mode == 1 ? "模式: 轨迹采样" : "模式: 坐标轴+箭头", new Vector2(12f, 30f), new Color(155, 190, 225, 220), 12f);
        g.Text("演示 ToScreen / DrawAxes / DrawArrow 常见组合", new Vector2(12f, 48f), new Color(130, 150, 175, 220), 12f);
    }
    ...
    private void DrawArrowRecipes(ICanvas2D g, Vector2 center, float scale)
    {
        var t = _ctx is null ? 0f : _ctx.Time;
        var vectors = new[]
        {
            new Vector2(MathF.Cos(t * 0.8f) * 1.1f, MathF.Sin(t * 0.8f) * 0.7f),
            new Vector2(MathF.Cos(t * 1.1f + 1.3f) * 0.8f, MathF.Sin(t * 1.1f + 1.3f) * 1.0f),
            new Vector2(MathF.Cos(t * 1.5f + 2.4f) * 0.6f, MathF.Sin(t * 1.5f + 2.4f) * 0.55f)
        };
        var colors = new[]
        {
    ...
    private void DrawOrbitSamples(ICanvas2D g, Vector2 center, float scale)
    {
        if (_ctx is null)
        {
            return;
        ...
        while (i < 20)
        {
            ...
            var p = new Vector2(MathF.Cos(t) * 1.1f, MathF.Sin(t * 1.4f) * 0.6f);
        ...
        var current = new Vector2(MathF.Cos(_ctx.Time) * 1.1f, MathF.Sin(_ctx.Time * 1.4f) * 0.6f);
