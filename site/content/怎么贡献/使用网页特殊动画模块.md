...
本文介绍站点的 `animts` 网页动画模块：用 C# 编写可视化动画（弹幕、NPC 对话、Boss/NPC 的 AI 分析动画、数学可视化等），在构建时编译成 JS，并在文章中以“特殊语法”嵌入到 `/site/pages/viewer.html`。
...
```animts
anims/demo-math-transform.cs
```

NPC/Boss AI 动画（克苏鲁之眼风格行为）：

```animts
anims/demo-eoc-ai.cs
```
...
- 用“选择仓库目录”选中项目根目录（或 `site/content/` 目录），从下拉框选择 `anims/*.cs`，再点“播放所选/复制 animts 代码块”。
