...
当前站点不再维护 `.cs -> .generated.md` 的自动生成流程。

如果你希望在教程里展示 C# 代码，推荐使用这两种方式：

1. 在 Front Matter 写 `source_cs`，在阅读页底部显示完整源码折叠区。
2. 在正文里用 `{{cs:...#cs:...|标题}}` 抽取指定类型/方法/字段片段。

具体语法和示例请看：`site/content/怎么贡献/站点Markdown扩展语法说明.md`。

你可以先打开 `site/pages/article-studio.html` 在浏览器里写 Markdown 草稿与预览。该页面不支持编辑 C# / 动画脚本；需要真实 tModLoader C# API 开发时，请先 Clone 仓库并在本地 IDE 中完成。
...
```
{{ref:文章使用内容索引/CSharp/DPapyru-CSharp从零到能看懂Mod代码.md|C#：最小示例}}
```
...
{if C == 0}
{{ref:./_分流/xxx-C0.md|C# 补课（C0）}}
{end}

{if T == 0}
{{ref:./_分流/xxx-T0.md|tModLoader 补课（T0）}}
{end}

{if C >= 1 && T >= 1}
{{ref:./_分流/xxx-advanced.md|进阶建议（C≥1 且 T≥1）}}
{end}
```
