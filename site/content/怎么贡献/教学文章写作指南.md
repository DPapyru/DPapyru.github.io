...
如果你只想查“站点支持哪些 Markdown 扩展语法”，请看：`site/content/怎么贡献/站点Markdown扩展语法说明.md`。

## 你要做的事情（最短路径）

1. 在 `site/content/` 下新增或修改 `*.md`（必须带 YAML Front Matter，至少 `title`）
2. 运行构建：`npm run build`（会自动更新 `site/content/config.json`、`site/assets/search-index.json`、`site/sitemap.xml`）
3. 运行一致性检查：`npm run check-generated`（确保生成文件已提交）
4. 本地预览（任选其一）：
   - `python -m http.server 8000`
   - `npx http-server -p 8080`

注意：`site/content/config.json`、`site/assets/search-index.json`、`site/sitemap.xml` 是自动生成的，请不要手改它们作为最终结果。

## 仓库结构速览

- `site/content/`：教程内容与站内页面（文章在 `site/content/**/**/*.md`）
- `site/assets/`：静态资源（图片建议放 `site/assets/imgs/`）
- 构建脚本：`site/tooling/generate-index.js`（生成索引/搜索/站点地图）
...
1. **分类由目录决定**：文章的“分类”来自文件所在目录，例如 `site/content/怎么贡献/`、`site/content/Modder入门/`，不要在 YAML 里写 `category`。
...
## 用 C# 写文章（.cs -> .generated.md）

站点支持一种“实验性写法”：贡献者直接在 `site/content/**` 下编写 `.cs` 文件，写正常 C# 代码，并用少量约定把它渲染成文章（生成 `*.generated.md`）。

适合场景：
- 教程需要大量可编译的示例代码，并希望 IDE 自动补全/重构
- 文章结构希望和代码结构同步维护（正文与示例代码穿插）

### 1）元数据怎么写（用特性 Attribute）

`.cs` 文件里需要在某个 `class` 上标记元数据（至少要有 `Title`，否则不会生成文章）：

```csharp
using ModDocProject;

namespace YourNamespace {
    /// <summary>一行简介（会作为 description 的候选）</summary>
    [Title("文章标题")]
    [Tooltip("文章描述")]
    [Author("你的名字")]
...
说明：
- `Tooltip` 会映射到 YAML 的 `description`
- `UpdateTime` 会映射到 YAML 的 `last_updated`
- `PrevChapter` / `NextChapter` 建议填写“类名”，并保持与文件名一致（站点会按同目录文件生成跳转）
- `PrevChapter` / `NextChapter` 支持两种写法：
  - 字符串：`[PrevChapter("CSharp快速入门")]`
  - Type：`[PrevChapter(typeof(CSharp快速入门))]`（或 `nameof(CSharp快速入门)`）

### 2）正文怎么写（#if DOCS + DocMarkdown* + #region）

正文必须写在 `#if DOCS` 块内，并且使用 `public const string DocMarkdown... = """..."""` 的形式。

重要规则：
- 变量名必须以 `DocMarkdown` 开头（例如 `DocMarkdown_1`、`DocMarkdown_作用域` 都可以）
- 正文标题层级由 `#region` 自动生成：在 `#if DOCS` 内写 `#region 标题` 会变成 `## 标题`
...
当前版本不支持把正文写成 `/** ... */` 注释块（只会提取 `DocMarkdown*` 字符串内容）。

### 3）示例代码怎么展示（把 #region 放在你希望出现的位置）

示例代码不是“统一放在文章末尾”，而是会按你在 `.cs` 文件里的位置，直接插入到正文中。

写法：
- 在 `#if DOCS` 外写 `#region 示例代码：...`，生成器会渲染成：
  - `### 示例代码：...`
  - 一段 `csharp` 代码块
- `#region 元数据` 会被视为内部结构，不会出现在文章里（避免把 Attribute 当正文展示）
...
生成结果：
- 每个 `.cs` 会生成同目录的 `*.generated.md`

本地预览（任选其一）：
- `python -m http.server 8000`
- 用浏览器打开 `site/pages/viewer.html?file=...generated.md`

## 文件位置与命名

推荐：`site/content/<分类>/作者-标题.md`

示例：

- `site/content/Modder入门/DPapyru-第一个武器.md`
- `site/content/怎么贡献/YourName-如何写一篇教程.md`
...
`site/content/Modder入门/` 采用“三层结构”写作（便于复用与分流）：

1. **指南层**：`site/content/Modder入门/指南/`  
   放“阅读方式/提问/环境/术语/约定”等通用说明，分流尽量少。
2. **内容索引层**：`site/content/Modder入门/文章使用内容索引/`  
   放可复用的 C# 补课与 tModLoader API 说明，**一文件一主题，多段落标题**；分流主要写在这里。
3. **组装层**：`site/content/Modder入门/`（或其子目录）  
   放路线页、章节页、导读页。它们尽量短，主要通过“引用展开”把索引层的段落插入进来。

### 内容索引的推荐段落（稳定标题）

为了让“引用展开”稳定可维护，索引文章建议固定包含这些段落标题（可按需要增减，但标题尽量不要随意改动）：
...
- 深度：使用 `C`（C# 熟练度）与 `T`（tML 熟练度）分流，例如 `{if C == 0}` / `{else if C == 1}` / `{else}` / `{end}`。
- 偏好：使用 `P_*` 分流（例如 `P_step`、`P_code`、`P_troubleshoot`、`P_api_reference`、`P_rendering`）。  
  偏好分流通常是“加法型内容”，建议用多个独立 `{if ...}{end}` 块，而不是写在同一个 `else if` 链里。

### 组装页的引用展开写法（推荐）

在组装页里，用“整行指令”插入索引文章的某个标题段落：

```
{[文章使用内容索引/CSharp/DPapyru-CSharp从零到能看懂Mod代码.md#最小示例（可引用）][C#：最小示例]}
```

更多语法细节与限制说明，请看：`site/content/怎么贡献/站点Markdown扩展语法说明.md`。

## 链接与图片（推荐写法）

- 同目录章节互链：直接写文件名，例如 `[下一章](下一章.md)`
- 图片建议放 `site/assets/imgs/`，在 Markdown 里用相对路径引用：
...
站点支持把 `quiz` 代码块渲染成可交互题目，渲染逻辑在 `site/assets/js/site-quiz.js`。
...
分流指令与引用语法的详细说明见：`site/content/怎么贡献/站点Markdown扩展语法说明.md`。
...
推荐直接参考：`site/content/Modder入门/1-入门/DPapyru-第一个武器.md`。
...
`/site/pages/viewer.html` 已统一提供页脚导航按钮，并会移除正文内重复的“章节进度”块。建议：
...
站点会根据入站测评与读者手动设置推导 `P_*` 偏好标签（见 `site/content/怎么贡献/站点Markdown扩展语法说明.md`）。
...
你可以在 `/site/pages/viewer.html` 开启作者模式（侧栏按钮或 URL `&author=1`），用来检查：
...
- [ ] 文章放在 `site/content/<分类>/` 下，文件名可读且不易踩坑
- [ ] 链接与图片使用相对路径，本地预览能打开
