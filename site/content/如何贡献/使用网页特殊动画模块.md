...
time: 35分钟
description: 在站点中使用 animts 模块，包含本地 Dotnet 实时预览桥接、模块索引与排错流程
...
1. 源码：`site/content/**/.anim.ts`
2. 编译产物：`site/assets/anims/*.js`
3. 清单：`site/assets/anims/manifest.json`
4. 运行时：`site/assets/js/animts-runtime.js`

文章中始终引用 `.anim.ts` 路径，不直接写 `.js`。
...
```bash
dotnet run --project site/tooling/tools/animts-preview-bridge
```
...
1. `anims/vector-basic.anim.ts` -> `anims/vec2-basic-ops.anim.ts`
2. `anims/vector-add-resolution.anim.ts` -> `anims/vec2-project-decompose.anim.ts`
3. `anims/matrix-mat4-transform.anim.ts` -> `anims/mat4-trs-compose.anim.ts`
4. `anims/demo-math-transform.anim.ts` -> `anims/mat4-view-projection.anim.ts`
5. `anims/vector-move.anim.ts` -> `anims/animgeom-toolkit-recipes.anim.ts`
...
```bash
ANIMTS_AST_INTEGRATION=1 node --test site/tooling/scripts/animts-compiler-ast.test.js
```

## 常见错误排查

1. `sourcePath 必须是 **/*.anim.ts`

原因：路径不在 `anims/` 或不是 `.anim.ts`。

2. 预览不更新但桥接在线

先确认你编辑的是 `**/*.anim.ts`，不是 `code/*.anim.ts`。
