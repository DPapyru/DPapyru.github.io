...
接着运行: `npm run build`（或 `node generate-index.js`）生成/更新：

- `docs/config.json`
- `sitemap.xml`
- `assets/search-index.json`

如果你修改了 `docs/**/*.md`，请在提交前运行 `npm run build` 并把以上生成文件一并提交（CI 会在 PR 中检查一致性）。
...
│   ├── folder.html        # 文件夹浏览页
│   ├── viewer.html        # Markdown阅读页
│   └── config.json        # 文档索引与元数据（由脚本生成）
├── generate-index.js       # 生成 docs/config.json
...
3. **进行开发**并遵循[贡献指南](docs/怎么贡献/DPapyru-贡献者如何编写文章基础.md)
...
我们欢迎各种形式的贡献！请阅读我们的[贡献指南](docs/怎么贡献/DPapyru-贡献者如何编写文章基础.md)了解详细信息。
