export function updateAI(state: any, input: any, dt: number) {
    ...
    if (state.phase === 1 && state.hp <= 0.55) state.phase = 2;
    ...
    for (let wi = 0; wi < worms.length; wi++) {
        const w = worms[wi];
        if (!w || !w.head || !w.vel || !Array.isArray(w.segments)) continue;
...
export function render(state: any, input: any, gfx: any) {
    if (!state || typeof state !== 'object') return;
    if (!gfx || !gfx.g) return;

    const g: CanvasRenderingContext2D = gfx.g;
    ...
    for (let wi = 0; wi < worms.length; wi++) {
        const w = worms[wi];
        if (!w || !w.head || !Array.isArray(w.segments)) continue;
