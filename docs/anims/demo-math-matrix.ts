...
type Mat2 = { a: number; b: number; c: number; d: number }; // [[a,b],[c,d]]

function vec(x: number, y: number): Vec2 { return { x, y }; }

function mulMatVec(m: Mat2, v: Vec2): Vec2 {
    return { x: m.a * v.x + m.b * v.y, y: m.c * v.x + m.d * v.y };
}

function drawMatrixText(g: CanvasRenderingContext2D, m: Mat2, x: number, y: number) {
    const lines = [
...
export default async function run(ctx: AnimContext) {
    ...
    let m: Mat2 = { a: 1, b: 0.2, c: 0.0, d: 1 };
    let unitPx = 70;
    let showGrid = true;
    ...
    ctx.ui.runLoop({
        ...
        draw: function () {
            ...
            function poly(points: Vec2[], fill: string, stroke: string) {
                g.save();
                g.fillStyle = fill;
