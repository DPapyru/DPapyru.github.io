...
export type Panel = {
    ...
    addToggle: (label: string, initial: boolean, onChange: (value: boolean) => void) => HTMLButtonElement;
    addSlider: (label: string, options: { min: number; max: number; step?: number; initial: number }, onChange: (value: number) => void) => HTMLInputElement;
};
...
export function createPanel(
    ...
    if (portalMode) panel.classList.add('animts-ui-panel--portal');
    ...
    const btnPin = el('button', 'btn btn-small btn-outline', '固定') as HTMLButtonElement;
    const btnCollapse = el('button', 'btn btn-small btn-outline', '折叠') as HTMLButtonElement;
    btnPin.type = 'button';
    ...
    function getPersistKey() {
        ...
        let scope = '';
        try {
            const embed = stage.closest && stage.closest('.animts-embed[data-animts-src]') ? stage.closest('.animts-embed[data-animts-src]') as HTMLElement : null;
            if (embed) {
    ...
    function clampToPage(left: number, top: number, width: number, height: number) {
    ...
    function setPinned(next: boolean) { pinned = !!next; apply(); }
    function togglePinned() { pinned = !pinned; apply(); return pinned; }
    function isCollapsed() { return collapsed; }
    function setCollapsed(next: boolean) { collapsed = !!next; apply(); }
    ...
    (function enableDragging() {
        ...
        function eventPageX(event: PointerEvent) {
            const x = (event as any).pageX;
            if (typeof x === 'number') return x;
            return event.clientX + (window.scrollX || 0);
        }

        function eventPageY(event: PointerEvent) {
            const y = (event as any).pageY;
        ...
        function onUp(event: PointerEvent) {
            if (!dragging) return;
            dragging = false;
            try { title.releasePointerCapture(event.pointerId); } catch (_) { }
    ...
    (function attachAutoCleanup() {
        if (!portalMode) return;
        const signal = (stage as any).__ANIMTS_SIGNAL;
    ...
    function addButton(label: string, onClick: () => void) {
        const row = addRow();
        const btn = el('button', 'btn btn-small btn-outline', label) as HTMLButtonElement;
        btn.type = 'button';
    ...
    function addToggle(label: string, initial: boolean, onChange: (value: boolean) => void) {
        let value = !!initial;
        const row = addRow();
        const btn = el('button', 'btn btn-small btn-outline', '') as HTMLButtonElement;
        btn.type = 'button';
        function sync() {
    ...
    function addSlider(
        label: string,
        sliderOptions: { min: number; max: number; step?: number; initial: number },
